...
def create_pull_request_for_issue(token, **kwargs):
    ...
    try:
        eyes_reaction_id = github_api.create_issue_reaction(token, kwargs['owner'], kwargs['repo_name'], kwargs['issue']['number'], "eyes")

        repo_dir = git_commands.clone_repository(token, temp_dir, kwargs['owner'], kwargs['repo_name'])

        branch_name = f"fix-issue-{kwargs['issue']['number']}"
        git_commands.checkout_new_branch(repo_dir, branch_name)

        files_list = extract_files_list_from_issue(kwargs['issue']['body'])

        # Prepare the prompt
        issue_pr_prompt = f"Please help me resolve this issue.\n\nIssue Title: {kwargs['issue']['title']}\n\nIssue Body: {kwargs['issue']['body']}"

        coding_result = aider_coder.do_coding_request(issue_pr_prompt, files_list, repo_dir)

        main_branch = github_api.get_default_branch(token, kwargs['owner'], kwargs['repo_name'])
        ...
        pr = github_api.create_pull_request(
            ...
            kwargs['owner'],
            kwargs['repo_name'],
            f"Fix issue #{kwargs['issue']['number']}: {coding_result['commit_message']}",
            f"This PR addresses the changes requested in issue #{kwargs['issue']['number']}\n\n{coding_result['summary']}",
            branch_name,
            main_branch
        )
        if not pr:
        ...
        github_api.create_issue_comment(token, kwargs['owner'], kwargs['repo_name'], kwargs['issue']['number'], comment_body)
        github_api.delete_issue_reaction(token, kwargs['owner'], kwargs['repo_name'], kwargs['issue']['number'], eyes_reaction_id)
        github_api.create_issue_reaction(token, kwargs['owner'], kwargs['repo_name'], kwargs['issue']['number'], "rocket")
...
def handle_pr_review_comment(token, **kwargs):
    ...
    if kwargs['comment']['user']['login'] == APP_USER_NAME:
        logger.info(f"Ignoring comment from {APP_USER_NAME}")
        return {"message": "Comment from app user ignored"}, 200

    pr_review_comment_id = kwargs['comment']['id']
    eyes_reaction_id = github_api.create_pr_review_comment_reaction(token, kwargs['owner'], kwargs['repo_name'], pr_review_comment_id, "eyes")

    if 'LGTM' in kwargs['comment']['body']:
    ...
    try:
        ...
        issue_number = extract_issue_number_from_pr_title(kwargs['pull_request']['title'])
        logger.info(f"Extracted issue number: {issue_number}")

        issue = github_api.get_issue(token, kwargs['owner'], kwargs['repo_name'], issue_number)
        logger.info(f"Retrieved issue: {issue['number'] if issue else None}")

        # Get the PR diff
        pr_diff = github_api.get_pr_diff(token, kwargs['owner'], kwargs['repo_name'], kwargs['pull_request']['number'])
        logger.info(f"Retrieved PR diff: {'Success' if pr_diff else 'Failed'}")

        # Build the prompt
        prompt = aider_coder.build_pr_review_prompt(issue, pr_diff, kwargs['comment']['body'])

        # Create a temporary directory
        temp_dir = tempfile.mkdtemp(dir=os.getcwd(), prefix='repo_')
        repo_dir = git_commands.clone_repository(token, temp_dir, kwargs['owner'], kwargs['repo_name'], kwargs['pull_request']['head']['ref'])

        # Get the list of files changed in the PR
        changed_pr_files = github_api.get_pr_changed_files(token, kwargs['owner'], kwargs['repo_name'], kwargs['pull_request']['number'])
        mentioned_files = extract_files_list_from_issue(kwargs['comment']['body'])
        ...
        git_commands.push_changes_to_repository(temp_dir, kwargs['pull_request']['head']['ref'])

        pr_comment_body = f"I've updated the PR based on the review comment.\n\n{coding_result['summary']}"

        github_api.reply_to_pr_review_comment(token, kwargs['owner'], kwargs['repo_name'], kwargs['pull_request']['number'], pr_review_comment_id, pr_comment_body)

        github_api.delete_pr_review_comment_reaction(token, kwargs['owner'], kwargs['repo_name'], pr_review_comment_id, eyes_reaction_id)
        github_api.create_pr_review_comment_reaction(token, kwargs['owner'], kwargs['repo_name'], pr_review_comment_id, "rocket")
